...
def predict_rating(target_user, target_movie, similarity_func, rating_matrix):
    ...
    if not similarities:
        logger.warning(f"No positive similarities found for user {target_user} and movie {target_movie}. Using average rating.")
    ...
    logger.debug(f"Predicted rating for user {target_user} and movie {target_movie}: {predicted_rating:.4f}")
